...
if(!function_exists('clean_export')){
	function clean_export($val) : string {
		return preg_replace(['/\&lt;\?php\&nbsp;/','/\?\&gt;/'],'',highlight_string('<?php '.var_export($val,true).' ?>',true));
	}
}
...
if(!function_exists('clean_dump')){
	function clean_dump($val) : string {
		...
		return preg_replace(['/\&lt;\?php\&nbsp;/','/\?\&gt;/'],'',highlight_string('<?php '.trim(ob_get_clean()).' ?>',true));
	}
}

if(!function_exists('clean_print')){
	function clean_print($val) : string {
		return preg_replace(['/\&lt;\?php\&nbsp;/','/\?\&gt;/'],'',highlight_string('<?php '.print_r($val,true).' ?>',true));
	}
}
